...
model1 = ChatGoogleGenerativeAI(model="gemini-3.1-flash-lite-preview")
...
prompt1 = PromptTemplate(
    template="You are a helpful assistant expert in the field of {field}. You are given detailed information about a topic and you have to generate notes for students. Here is the detailed information: {information}",
    input_variables=["field", "information"]
)

prompt2 = PromptTemplate(
    template="You are a helpful assistant expert in the field of {field}. You are given detailed information about a topic and you have to generate a quiz based on that information. Here is the information: {information}",
    input_variables=["field", "information"]
)

# Template 3
prompt3 = PromptTemplate(
    template="You are given lecture notes : {notes} and practice quiz : {quiz} and you have to merge them into a single document ",
    input_variables=["notes", "quiz"]
)

# Generate chain for generating notes and quiz in parallel and then merging them

# 1. Create a parallel chain to generate notes and quiz in parallel
...
#2. Create a merge chain to merge the notes and quiz into a single document
merge_chain = prompt3 | model1 | parser

# 3. Create a final chain to run the parallel chain and then merge the results
...
"""
result = chain.invoke({"field": "Machine Learning", "information": information})
print(result)
